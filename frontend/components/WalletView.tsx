...
function AnimatedBalance({ value }: { value: number }) {
  ...
  return <Text style={s.heroAmount}>₹{display.toLocaleString('en-IN')}</Text>;
}
...
export default function WalletView({ onClose }: WalletViewProps) {
  ...
  // ── Real state — all zeros until API responds ─────────────────────────
  ...
  const loadWallet = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        setOutstandingDues(d.outstanding_dues || 0);
        ...
        // Compute today earned from transaction list
        const today = new Date().toDateString();
        const te = txList
          .filter((t: any) => t.type === 'credit' && new Date(t.date).toDateString() === today)
          .reduce((a: number, b: any) => a + b.amount, 0);
        setTodayEarned(te || d.today_earnings || 0);
        setTotalTrips(d.total_trips || 0);
      }
    } catch (e) {
  ...
  // ── Add money (top-up) ────────────────────────────────────────────────
  // Note: most platforms handle top-up via a payment gateway URL redirect.
  // The API stores the result. For now we simulate with a local update and
  // inform the user to integrate Razorpay / Cashfree as needed.
  const handleAddMoney = async () => {
    const amt = parseFloat(topupAmount);
    if (!amt || amt < 1) return Alert.alert('Invalid Amount', 'Please enter a valid amount.');
    setIsLoading(true);
    try {
      // In production: open Razorpay/Cashfree payment sheet here, then call
      // WalletAPI.addMoney(amt, paymentMethod) with the payment token.
      // For now we do an optimistic local update and reload.
      Alert.alert(
        '💳 Payment Gateway',
        'Integrate Razorpay or Cashfree here to process the payment. The wallet will be credited automatically via webhook.',
        [{ text: 'OK', onPress: () => { setIsAddMoneyOpen(false); setTopupAmount(''); } }]
      );
    } catch (err: any) {
  ...
  const handlePayDues = async () => {
    ...
    setIsLoading(true);
    try {
      const result = await WalletAPI.payDues(amt);
      if (result.success) {
        await loadWallet(); // refresh from server
  ...
  const handleWithdraw = async () => {
    ...
    if (!upiId.trim())       return Alert.alert('UPI ID Required', 'Please enter your UPI ID.');

    setIsLoading(true);
    try {
      const result = await WalletAPI.withdraw(amt, upiId.trim());
      if (result.success) {
        await loadWallet(); // refresh balance from server
        setIsWithdrawOpen(false);
        setWithdrawAmount('');
        setUpiId('');
        Alert.alert('🏦 Withdrawal Initiated', `₹${amt.toLocaleString()} will reach your bank in 1–2 business days.`);
      } else {
        Alert.alert('Withdrawal Failed', result.message || 'Please try again.');
      }
    } catch (err: any) {
  ...
  return (
    <View style={s.root}>
      ...
      <SafeAreaView style={s.header}>
        ...
        <TouchableOpacity onPress={loadWallet} style={s.backBtn}>
      ...
      {loadingWallet ? (
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
          ...
          <Text style={{ color: '#64748B', marginTop: 12, fontWeight: '600' }}>Loading wallet...</Text>
        </View>
      ) : (
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll}>
          ...
          <View style={s.statsRow}>
            {[
              ...
              { label: 'Total Trips',  value: String(totalTrips),                         icon: 'package',      color: '#0284C7', bg: '#DBEAFE' },
          ...
          <View style={s.txHeader}>
            ...
            <TouchableOpacity style={s.txFilterBtn} onPress={loadWallet}>
              <Feather name="refresh-cw" size={14} color="#0284C7" />
              <Text style={s.txFilterText}>Refresh</Text>
            </TouchableOpacity>
          </View>
          ...
          {transactions.length === 0 ? (
            ...
            return (
              <View key={t.id || idx} style={[s.txItem, idx === 0 && s.txItemFirst]}>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={s.overlay}>
          ...
          <View style={s.sheet}>
            ...
            <View style={s.bigInputRow}>
              ...
              <TouchableOpacity onPress={() => setPayAmount(Math.ceil(outstandingDues).toString())} style={s.fullBtn}>
                <Text style={s.fullBtnText}>Full</Text>
              </TouchableOpacity>
            </View>

            <View style={s.quickRow}>
              {['100', '200', Math.ceil(outstandingDues).toString()].map(a => (
                <TouchableOpacity key={a} onPress={() => setPayAmount(a)} style={[s.quickChip, payAmount === a && s.quickChipActive]}>
          ...
          <View style={s.sheet}>
            ...
            <View style={[s.quickRow, { marginTop: 14 }]}>
              {['500', '1000', '2000', '5000'].map(a => (
                <TouchableOpacity key={a} onPress={() => setWithdrawAmount(a)} style={[s.quickChip, withdrawAmount === a && s.quickChipActive]}>
                  <Text style={[s.quickChipText, withdrawAmount === a && s.quickChipTextActive]}>₹{a}</Text>
                </TouchableOpacity>
              ))}
            </View>

            {/* UPI ID input for real withdrawal */}
            <Text style={[s.inputLabel, { marginTop: 4 }]}>YOUR UPI ID</Text>
            <View style={[s.bigInputBox, { marginBottom: 16 }]}>
              <Feather name="smartphone" size={20} color="#94A3B8" style={{ marginRight: 8 }} />
              <TextInput
          ...
          <View style={s.sheet}>
            {paymentSuccess ? (
              ...
              <>
                ...
                <View style={[s.quickRow, { marginTop: 14 }]}>
                  {['500', '1000', '2000', '5000'].map(a => (
                    <TouchableOpacity key={a} onPress={() => setTopupAmount(a)} style={[s.quickChip, topupAmount === a && s.quickChipActive]}>
...
const s = StyleSheet.create({
  ...
  // Hero Card
  heroCard:       { borderRadius: 28, padding: 24, marginBottom: 16, overflow: 'hidden', position: 'relative', shadowColor: '#0F172A', shadowOffset: { width: 0, height: 12 }, shadowOpacity: 0.4, shadowRadius: 20, elevation: 12 },
  ...
  // Dues Card
  duesCard:       { backgroundColor: '#FFF', borderRadius: 22, padding: 18, marginBottom: 16, borderWidth: 2, borderColor: '#FEF3C7', shadowColor: '#D97706', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.08, shadowRadius: 10, elevation: 3 },
  ...
  statBox:     { flex: 1, backgroundColor: '#FFF', borderRadius: 18, padding: 14, alignItems: 'center', gap: 6, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 5, elevation: 2 },
  ...
  txItem:       { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFF', borderRadius: 18, padding: 14, marginBottom: 10, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 4, elevation: 1 },
  ...
  bigInputBox: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F8FAFC', borderWidth: 2, borderColor: '#E2E8F0', borderRadius: 18, paddingHorizontal: 18, marginBottom: 0 },
  ...
  bankRow:     { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F8FAFC', borderRadius: 16, padding: 14, marginBottom: 20, borderWidth: 1, borderColor: '#E2E8F0', gap: 12 },
  ...
  primaryBtn:     { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, backgroundColor: '#0284C7', paddingVertical: 18, borderRadius: 18, marginBottom: 12, shadowColor: '#0284C7', shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.3, shadowRadius: 12, elevation: 6 },
  ...
  successIcon:  { width: 104, height: 104, borderRadius: 52, justifyContent: 'center', alignItems: 'center', marginBottom: 26, shadowColor: '#22C55E', shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.4, shadowRadius: 20, elevation: 12 },
