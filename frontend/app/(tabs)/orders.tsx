...
import {
  ...
  Modal, Dimensions, TextInput, Alert, Linking, Platform, ActivityIndicator
} from 'react-native';
...
export default function OrdersScreen() {
    ...
    try {
      ...
      if (result.success) {
        const normalized = (result.data || []).map((t: any) => ({
          ...
          status: status,
          date: t.completed_at ? new Date(t.completed_at).toLocaleDateString('en-IN') : '',
  ...
  const handleAccept = async (order: any) => {
    try {
      ...
      // Optimistic — remove from list and proceed
      setAvailableOrders(prev => prev.filter(o => o.id !== order.id));
      setNavigatingOrder(order);
    }
  };
  ...
  const handleOpenCamera = async () => {
    ...
    const result = await ImagePicker.launchCameraAsync({ mediaTypes: ImagePicker.MediaTypeOptions.Images, allowsEditing: true, aspect: [4, 3], quality: 0.85 });
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      if (result.success) {
        Alert.alert('Delivery Completed! 🎉', `₹${result.payout || activeTrip?.payout || 0} has been added to your Wallet.`);
        setHistoryOrders(prev => [{ ...(activeTrip || {}), status: 'Completed', date: new Date().toLocaleDateString('en-IN') }, ...prev]);
