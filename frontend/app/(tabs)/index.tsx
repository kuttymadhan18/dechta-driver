...
import { EarningsAPI, DriverAPI, DriverStore, OrdersAPI } from "../../services/api";
...
export default function HomeScreen() {
  ...
  const loadDashboardData = async () => {
    try {
      // Load driver name from local store first (instant)
      const driver = await DriverStore.get();
      if (driver?.fullName) {
        setDriverName(driver.fullName.split(" ")[0] || "Partner");
      }

      // Fetch live summary + notifications in parallel
      const [summaryRes, notifRes] = await Promise.allSettled([
      ...
      if (notifRes.status === "fulfilled" && notifRes.value.success) {
        setAlerts(notifRes.value.data?.slice(0, 5) || []);
      }

      // Fetch promo slides
      try {
        const { MiscAPI } = await import("../../services/api");
        const promoRes = await MiscAPI.getPromos();
  ...
  const acceptOrder = async (id: string) => {
    try {
      ...
      const acceptedData = result.success ? result.trip : ringingOrder;
      setRingingOrder(null);
      router.push({
        pathname: "/orders",
        params: { incomingOrder: JSON.stringify(acceptedData || ringingOrder) },
      });
    } catch (_) {
      // On error still navigate with local order data
      const acceptedData = ringingOrder;
      setRingingOrder(null);
      router.push({
        pathname: "/orders",
        params: { incomingOrder: JSON.stringify(acceptedData) },
      });
    }
  };
    ...
    <SafeAreaView style={[styles.container, theme.container]}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.promoSlider}>
          {promoMedia.length > 0 && (
            <Image source={{ uri: promoMedia[currentSlide % promoMedia.length] }} style={styles.promoImg} />
          )}
